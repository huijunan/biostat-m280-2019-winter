---
title: "Biostat M280 Homework 1 by Huijun An"
...
/home/m280data/NYCHVS/NYCHVS_2017.csv
  ...
  The purpose of the script is to display all the file names with the extension ".csv" in the directory "/home/m280data/NYCHVS/".
...
**Solution**
```{bash}
cat /home/m280data/NYCHVS/NYCHVS_1991.csv | cut -d, -f 2| sort | uniq -c
```
...
**Solution**
```{bash}
for str in {Elizabeth,Darcy,Jane,Lydia}
  do
  cat pride_and_prejudice.txt|grep -o $str|uniq -c
  done
```
...
**Solution**
  ...
  The first command with ">" will overwrite the original content in test1.txt with 'hello, world'; whereas the second command with ">>" will add 'hello, world' to the original content in test2.txt.
...
**Solution**
  ...
  The output will show the last 5 lines from the first 20 lines of "pride_and_prejudice.txt", which are the 16th to 20th lines. 
...
Character set encoding: ASCII
  ...
  The `"$1"`, `"$2"`, and `"$3"` are positional parameters
"$1" means select the first argument 'pride_and_prejudice', 
"$2" means select the second argument '20', 
"$3" means select the third argument '5'
