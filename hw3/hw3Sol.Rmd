...
```{sql connection="citation"}
SELECT count(*), Issue_Year FROM latix 
...
```{sql connection="citation"}
SELECT count(*), Issue_Hour FROM latix GROUP BY Issue_Hour
ORDER BY count(*) DESC LIMIT 1
```
```{sql connection="citation"}
SELECT count(*), Issue_Wday FROM latix GROUP BY Issue_Wday
ORDER BY count(*) DESC LIMIT 1
```
```{sql connection="citation"}
SELECT count(*), Issue_Month FROM latix GROUP BY Issue_Month
ORDER BY count(*) DESC LIMIT 1
```
```{sql connection="citation"}
SELECT count(*), Issue_Day FROM latix GROUP BY Issue_Day
ORDER BY count(*) DESC LIMIT 1
```
...
```
```{sql connection="citation"}
SELECT count(*), Issue_Wday FROM latix 
WHERE Issue_Wday IS NOT NULL
GROUP BY Issue_Wday
ORDER BY count(*) ASC LIMIT 1
```
```{sql connection="citation"}
SELECT count(*), Issue_Month FROM latix 
WHERE Issue_Month IS NOT NULL
GROUP BY Issue_Month
ORDER BY count(*) ASC LIMIT 1
```
```{sql connection="citation"}
SELECT count(*), Issue_Day FROM latix 
...
```{sql connection="citation"}
SELECT count(*), Violation_Code, Violation_Description FROM latix 
...
```{sql connection="citation"}
SELECT sum(Fine_amount), Issue_Year FROM latix 
